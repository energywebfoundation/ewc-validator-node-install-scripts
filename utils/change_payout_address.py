...
parser.add_argument('--secret', help="Path of the encryption key", required=True)
parser.add_argument('--payoutAddress', help="Payout address", required=True)
parser.add_argument('--chain', help="Chain ID in hex format (default Volta)")
...
with open(args.secret, mode='r') as encryption_key:
    secret = encryption_key.readline()

# unlock and sign
key_files = os.listdir(args.keydir)
if len(key_files) > 1:
...
with open(pkey, mode='r') as keyfile:
    ...
    print('Tx hash: ' + bytes(tx_hash).hex())
